...
(ns cljs.core.async.impl.ioc-macros
  (:require-macros [cljs.core.async.impl.ioc-macros-m :refer [aset-all! gen-plan]])
  (:refer-clojure :exclude [all])
  (:require #?(:clj [clojure.pprint :refer [pprint]]
               :cljs [cljs.pprint :refer [pprint]])
            [clojure.set :refer (intersection)]
            #_[clojure.core.async.impl.protocols :as impl]
            #_[clojure.core.async.impl.dispatch :as dispatch]
            [cljs.analyzer :as cljs])
  #_(:import [java.util.concurrent.locks Lock]))
...
(defrecord XFn [fn-expr local-names local-refs]
  IInstruction
...
(defmethod sexpr-to-ssa 'fn*
  ...
  (gen-plan
   [locals (get-binding :locals)
    fn-id (add-instruction (->XFn fn-expr (keys locals) (vals locals)))]
   fn-id))
